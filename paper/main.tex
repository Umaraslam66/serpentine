...
\title{One Attention Layer Is Enough: Diagnosing and Repairing\\
State-Space Encoders for Routing under Reinforcement Learning}

\author{Umar Aslam\thanks{Independent Researcher. Correspondence: \texttt{aslamumar16@gmail.com}.}}
...
Mamba---is killed: at a matched 250k-step budget it reaches an 8.34\pct{}
single-trajectory optimality gap versus attention's 2.95\pct. Diagnostics
...
\begin{itemize}
  ...
  encoder reaches an 8.34\pct{} single-trajectory gap versus attention's
  2.95\pct{} (\S\ref{sec:kill}). The kill was pre-registered.
...
The encoders are matched in total model size (Table~\ref{tab:params}): attention
...
multistart, versus attention's 2.95\pct{} / 1.91\pct{} (Table~\ref{tab:main}).
The first clause of the kill rule is satisfied by a wide margin ($+5.4$ points
absolute); as \S\ref{sec:order} shows, the second clause resolves in an
unanticipated direction. The encoder-swap-as-specified is dead. We now ask
...
retraining ($n=200$). The gap moves from 8.38\pct{} (Hilbert, the training order)
...
14.6\pct{}$\rightarrow$22.2\pct{}), while the encoder's receptive field is fixed
($\sim$20). This is a geometry of the serialization, not a contribution of ours;
...
across three seeds, statistically on par with attention's budget-matched 1.91.
...
hybrid decisively beats attention on multistart. Seed 1 reaches 3.013\pct{}
single / 1.016\pct{} multistart versus attention's 2.454\pct{} / 1.589\pct;
across three seeds the hybrid is 3.08\,$\pm$\,0.06 / 1.14\,$\pm$\,0.19, and the
\emph{worst} hybrid seed's multistart (1.365) is still below the best attention
figure (1.589), well outside the $\pm$0.05 multistart eval-noise band. This is the
one place we use the word "beats," and we scope it: it is a multistart claim, with
the noise band and seed range stated. On single-trajectory, attention remains
...
inference on random-trained checkpoints we draw $k=8$ random orders and keep the
per-instance best tour. This lowers uni-Mamba/random multistart from 3.33\pct{} to
2.72\pct{} (average-start 6.42\pct{}$\rightarrow$5.01\pct{}) and
BiMamba/random from 3.31\pct{} to 2.70\pct. Setting $k=1$ reproduces the standard
eval exactly. Attention has no ordering degree of freedom, so this is a
...
over LKH-3), single-trajectory (multistart). Attention seeds 2--3 reached 440k at
walltime; their final 500k numbers are pending (resumed) and are labeled
accordingly. The multistart eval-noise band is $\pm$0.05.}
\label{tab:conv}
\begin{tabular}{ll cc}
\toprule
...
Attention & s2 (@440k)      & 2.778 & 1.732 \\
Attention & s3 (@440k)      & 2.589 & 1.673 \\
\midrule
...
The strongest 500k statement (1.016 vs 1.589) is seed-1 vs seed-1; the
seed-robust form is the 250k three-seed comparison and the 500k three-seed hybrid
mean whose worst seed still beats the best attention figure. Attention seeds 2--3
are finalizing from a 440k checkpoint at submission time. Multistart differences
below $\sim$0.05 are within eval noise and should not be read as wins.
